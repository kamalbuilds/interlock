...
Sam, a delivery operator at a boutique restoration house, clears a library through a
streamer's automated QC gate. The rejection slip says loudness failed. It does not say
where. Render-farm SRE dashboards in this hackathon lane watch CPU and queue depth.
They do not file a film's own loudness into Grafana Cloud and close the alert only on a
second measurement.

Interlock talks to Grafana **only** through the official `grafana/mcp-grafana` MCP
server. Remove that path and there is no folder, no dashboard, no annotation, no alert
rule, and no close. Measured close on Night Tide: repaired to **-23.1 LUFS**,
`landed=True`, Grafana's own alert state history showed Alerting then Normal. Honest
limit: each title is a stated window (120 s or 300 s), not a full-length scan.
...
**Grafana Cloud MCP is load-bearing, not a sidecar.** Interlock holds no Grafana HTTP
client. Every call is a tool call on `grafana/mcp-grafana`, held as a child process over
stdio. A demo that only painted a dashboard without MCP writes would fail this track; the five write classes below are the compliance proof. Five write classes land per run: `create_folder`, `update_dashboard`,
